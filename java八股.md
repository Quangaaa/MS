...
八大基本类型	byte/short/int/long
float/double
char
boolean	1. 包装类型 Integer 缓存池 [-128,127]，valueOf() 与 new 区别。
2. 自动装箱/拆箱底层是 Integer.valueOf() / intValue()。
3. float 精度丢失原理，BigDecimal 的正确使用 (compareTo vs equals)。
4. boolean 在 JVM 中的实际占用大小 (虚拟机未严格定义，多按 int 处理)。
引用类型	String	1. 不可变性：final char[]，常量池 (JDK7 后从方法区移入堆)。
2. String 拼接底层用 StringBuilder，但循环内拼接仍需显式使用 StringBuilder。
3. intern() 在不同 JDK 版本下的行为。
访问修饰符	private / default / protected / public	1. 访问权限精确表 (同类、同包、子类、任意)，尤其 protected 的子类可见性。
2. default (包级别) 是 Java 独特的设计，接口中 default 方法与此不同。
3. abstract 和 final 不能与 private 组合的原因。
关键字	static / final / transient / volatile / synchronized	1. static 变量在方法区 (元空间) 的存储，<clinit> 线程安全。
2. final 修饰类、方法、变量的区别，final 与不可变对象的关系。
3. transient 配合序列化，静态变量不会被序列化。
二、集合框架深度分析
...
GC 机制	可达性分析、标记-清除/复制/整理、分代回收	1. CMS (老年代并发清除，浮动垃圾、提前晋升等碎片化问题)。
2. G1 (Region 化、SATB、Mixed GC、Remembered Set)。
3. ZGC (染色指针，亚毫秒暂停)。
